...
TITLE_PRIORITY_UNPATCHED_112='else if(V.type==="custom-title"&&V.sessionId)O.set(V.sessionId,V.customTitle);else if(V.type==="tag"&&V.sessionId)T.set(V.sessionId,V.tag);else if(V.type==="agent-name"&&V.sessionId)$.set(V.sessionId,V.agentName);else if(V.type==="agent-color"&&V.sessionId)A.set(V.sessionId,V.agentColor);else if(V.type==="agent-setting"&&V.sessionId)z.set(V.sessionId,V.agentSetting);else if(V.type==="mode"&&V.sessionId)j.set(V.sessionId,V.mode);else if(V.type==="permission-mode"&&V.sessionId)D.set(V.sessionId,V.permissionMode);else if(V.type==="worktree-state"&&V.sessionId)M.set(V.sessionId,V.worktreeSession);else if(V.type==="pr-link"&&V.sessionId)w.set(V.sessionId,V.prNumber),Y.set(V.sessionId,V.prUrl),f.set(V.sessionId,V.prRepository);else if(V.type==="file-history-snapshot")J.set(V.messageId,V);else if(V.type==="attribution-snapshot")P.clear(),P.set(V.messageId,V);else if(V.type==="content-replacement")if(V.agentId){let C=R.get(V.agentId)??[];R.set(V.agentId,C),C.push(...V.replacements)}else{let C=X.get(V.sessionId)??[];X.set(V.sessionId,C),C.push(...V.replacements)}else if(V.type==="marble-origami-commit")W.push(V);else if(V.type==="marble-origami-snapshot")G=V'
TITLE_PRIORITY_PATCHED_112='else{let t=V.type,i=V.sessionId;if(t==="custom-title"&&i)O.set(i,V.customTitle),O.set("c"+i,1);else if(t==="ai-title"&&i&&!O.has("c"+i))O.set(i,V.aiTitle);else if(t==="tag"&&i)T.set(i,V.tag);else if(t==="agent-name"&&i)$.set(i,V.agentName);else if(t==="agent-color"&&i)A.set(i,V.agentColor);else if(t==="agent-setting"&&i)z.set(i,V.agentSetting);else if(t==="mode"&&i)j.set(i,V.mode);else if(t==="permission-mode"&&i)D.set(i,V.permissionMode);else if(t==="worktree-state"&&i)M.set(i,V.worktreeSession);else if(t==="pr-link"&&i)w.set(i,V.prNumber),Y.set(i,V.prUrl),f.set(i,V.prRepository);else if(t==="file-history-snapshot")J.set(V.messageId,V);else if(t==="attribution-snapshot")P.clear(),P.set(V.messageId,V);else if(t==="content-replacement")if(V.agentId){let C=R.get(V.agentId)??[];R.set(V.agentId,C),C.push(...V.replacements)}else{let C=X.get(i)??[];X.set(i,C),C.push(...V.replacements)}else if(t==="marble-origami-commit")W.push(V);else if(t==="marble-origami-snapshot")G=V;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;}'

# v2.1.116: minified renames in the chain — mode handler j→D, permission-mode D→f,
# pr-link prNumber w→Y, prUrl Y→w, prRepository f→j, content-replacement inner C→I.
TITLE_PRIORITY_UNPATCHED_116='else if(V.type==="custom-title"&&V.sessionId)O.set(V.sessionId,V.customTitle);else if(V.type==="tag"&&V.sessionId)T.set(V.sessionId,V.tag);else if(V.type==="agent-name"&&V.sessionId)$.set(V.sessionId,V.agentName);else if(V.type==="agent-color"&&V.sessionId)A.set(V.sessionId,V.agentColor);else if(V.type==="agent-setting"&&V.sessionId)z.set(V.sessionId,V.agentSetting);else if(V.type==="mode"&&V.sessionId)D.set(V.sessionId,V.mode);else if(V.type==="permission-mode"&&V.sessionId)f.set(V.sessionId,V.permissionMode);else if(V.type==="worktree-state"&&V.sessionId)M.set(V.sessionId,V.worktreeSession);else if(V.type==="pr-link"&&V.sessionId)Y.set(V.sessionId,V.prNumber),w.set(V.sessionId,V.prUrl),j.set(V.sessionId,V.prRepository);else if(V.type==="file-history-snapshot")J.set(V.messageId,V);else if(V.type==="attribution-snapshot")P.clear(),P.set(V.messageId,V);else if(V.type==="content-replacement")if(V.agentId){let I=R.get(V.agentId)??[];R.set(V.agentId,I),I.push(...V.replacements)}else{let I=X.get(V.sessionId)??[];X.set(V.sessionId,I),I.push(...V.replacements)}else if(V.type==="marble-origami-commit")W.push(V);else if(V.type==="marble-origami-snapshot")G=V'
TITLE_PRIORITY_PATCHED_116='else{let t=V.type,i=V.sessionId;if(t==="custom-title"&&i)O.set(i,V.customTitle),O.set("c"+i,1);else if(t==="ai-title"&&i&&!O.has("c"+i))O.set(i,V.aiTitle);else if(t==="tag"&&i)T.set(i,V.tag);else if(t==="agent-name"&&i)$.set(i,V.agentName);else if(t==="agent-color"&&i)A.set(i,V.agentColor);else if(t==="agent-setting"&&i)z.set(i,V.agentSetting);else if(t==="mode"&&i)D.set(i,V.mode);else if(t==="permission-mode"&&i)f.set(i,V.permissionMode);else if(t==="worktree-state"&&i)M.set(i,V.worktreeSession);else if(t==="pr-link"&&i)Y.set(i,V.prNumber),w.set(i,V.prUrl),j.set(i,V.prRepository);else if(t==="file-history-snapshot")J.set(V.messageId,V);else if(t==="attribution-snapshot")P.clear(),P.set(V.messageId,V);else if(t==="content-replacement")if(V.agentId){let I=R.get(V.agentId)??[];R.set(V.agentId,I),I.push(...V.replacements)}else{let I=X.get(i)??[];X.set(i,I),I.push(...V.replacements)}else if(t==="marble-origami-commit")W.push(V);else if(t==="marble-origami-snapshot")G=V;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;}'

# Sites 4–6 are version-specific (minified names change). Each version contributes
# its own site row; only one will match a given binary, the rest report "not found".
# v2.1.112 helpers: lW guard, LL setter, MH6 ai-title persist, b_ sessionId, Gw8 store-emit,
#                   $j renameTitle, X3 agentType, V isLoading.
# v2.1.116 helpers: wj guard, Yj setter, a86 ai-title persist, y_ sessionId, eO_ store-emit,
#                   hf renameTitle, ER agentType, E isLoading.
SITES=(
  ...
  "persistTitle_112|replace|(yL)=>{if(yL)LL(yL);else lW.current=!1}|(yL)=>{if(yL)MH6(b_(),yL),Gw8(yL);;;;;}|toolPermissionContext|120"
  "persistTitle_116|replace|(CV)=>{if(CV)Yj(CV);else wj.current=!1}|(CV)=>{if(CV)a86(y_(),CV),eO_(CV);;;;;}|toolPermissionContext|120"
  "titlePriority_112|replace|${TITLE_PRIORITY_UNPATCHED_112}|${TITLE_PRIORITY_PATCHED_112}|marble-origami-snapshot|1200"
  "titlePriority_116|replace|${TITLE_PRIORITY_UNPATCHED_116}|${TITLE_PRIORITY_PATCHED_116}|marble-origami-snapshot|1200"
  "titleGate_112|replace|!V&&!\$j&&!X3&&!lW.current|!V&&!!1&&!X3&&!lW.current|AbortController|250"
  "titleGate_116|replace|!E&&!hf&&!ER&&!wj.current|!E&&!!1&&!ER&&!wj.current|AbortController|250"
)
